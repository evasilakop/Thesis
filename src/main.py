...
def run_simulation(args):
    ...
    while True:
        detections, _ = detect_and_show(detector)
        ...
        add_vehicles_to_sumo(sumo, detections, args, depart_counter)
        aggregate_and_broadcast_weights(node, detections, args)
        step_simulation(sumo, args)

        total_nodes = args["nodes"]
        weights_array = get_weights_array(node, total_nodes)
        ...
        max_idx = define_max_idx(weights_array)
        broadcast_control_messages(sumo, node, total_nodes, max_idx)
        sumo.step()

def detect_and_show(detector):
    detections, frame = detector.detect_vehicles()
    if frame is not None:
        #cv2.imshow(f"Node {args['index']} View", frame)
        if cv2.waitKey(1) & 0xFF == ord('q'):
            return None, None
    return detections, frame

def add_vehicles_to_sumo(sumo, detections, args, depart_counter):
...
def aggregate_and_broadcast_weights(node, detections, args):
...
def step_simulation(sumo, args):
    for _ in range(int(args["frequency"]) * 5):
        time.sleep(0.2)
        sumo.step()

def get_weights_array(node, total_nodes):
    try:
        return np.array([
...
def define_max_idx(weights_array):
...
def broadcast_control_messages(sumo, node, total_nodes, max_idx):
